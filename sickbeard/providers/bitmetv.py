###################################################################################################
# Author:  <depraved22@gmail.com>
# URL: https://github.com/depraved22/Sick-Beard
#
...
class BitMeTVProvider(generic.TorrentProvider):
    ...
    def __init__(self):
        ...
        self.url = 'https://www.bitmetv.org/'
        logger.log("[" + self.name + "] initializing...")
    
    ###################################################################################################
    
    def isEnabled(self):
        return sickbeard.BITME
    ...
    def _doLogin(self):
        login_params  = {
            'username': sickbeard.BITME_USERNAME,
            'password': sickbeard.BITME_PASSWORD,
            'login': 'submit'
        }
        ...
        if re.search("Incorrect username or Password\. Please try again|<title>BITME\ORG \:\: You're home now",response.text) \
...
class BITMECache(tvcache.TVCache):
    ...
    def _getRSSData(self):
        xml = ''
        if sickbeard.BITME_RSSHASH:
            self.rss_url = "https://www.bitmetv.org/rss.php?&uid={0}&passkey={1}".format(sickbeard.BITME_USERNAME,sickbeard.BITME_RSSHASH)
            ...
            url = provider.url + "browse.php?&search="
